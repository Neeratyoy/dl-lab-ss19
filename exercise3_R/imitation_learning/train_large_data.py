...
def read_data(file_name, frac = 0.1):
    """
...
def train_model(data1, data2, n_minibatches, batch_size, lr, optimizer,
                history_length, freq, augment1, augment2, model_dir="./models", tensorboard_dir="./tensorboard"):
    frac = 0.1
    print("Creating Validation Set...")
    print("Phase I...")
    augment = "False"
    # read data
    X_train, y_train, X_valid, y_valid = read_data(data1, frac=frac)
    # preprocess data
    X_train, y_train, X_valid, y_valid = preprocessing(X_train, y_train, X_valid, y_valid,
                                                       history_length=history_length,
                                                       augment=augment1)
    valid_X = np.zeros((2*X_valid.shape[0], history_length, X_valid.shape[-2], X_valid.shape[-1]))
    valid_y = np.zeros((2*y_valid.shape[0]))
    valid_X[:len(X_valid)] = X_valid[:len(X_valid)].copy()
    valid_y[:len(y_valid)] = y_valid[:len(y_valid)].copy()
    del(X_train, y_train, X_valid, y_valid)
    print("Phase II...")
    # read data
    X_train, y_train, X_valid, y_valid = read_data(data2, frac=frac)
    # preprocess data
    X_train, y_train, X_valid, y_valid = preprocessing(X_train, y_train, X_valid, y_valid,
                                                       history_length=history_length,
                                                       augment=augment2)
    valid_X[-len(X_valid):] = X_valid[:len(X_valid)].copy()
    valid_y[-len(y_valid):] = y_valid[:len(y_valid)].copy()
    print("Validation Set Ready: ", valid_X.shape, valid_y.shape)
    del(X_train, y_train, X_valid, y_valid)
    ...
    for i in range(n_minibatches):
        if i % 2 == 0:
            ...
            X_train, y_train, X_valid, y_valid = read_data(data1, frac=frac)
            # preprocess data
            X_train, y_train, _, _ = preprocessing(X_train, y_train, X_valid, y_valid,
                                                               history_length=history_length,
                                                               augment=augment1)
        else:
            # read data
        ...
        valid_pred, valid_loss = agent.predict(valid_X, valid_y)
        ...
        valid_acc = balanced_accuracy_score(valid_y, valid_pred)
        eval_dict = {
